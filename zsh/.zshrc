...
,diff () {
    (cd $(git rev-parse --show-toplevel) && git diff $@ --color=always -- $(git diff $@ --name-only | fzf -m --ansi --preview "git diff $@ --color=always -- {-1} | diff-so-fancy" --preview-window=right:70%))
}
############
### PATH ###
############
